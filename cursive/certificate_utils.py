...
def can_sign_certificates(certificate, certificate_uuid=''):
    ...
    try:
        ...
        if not basic_constraints.ca:
            LOG.debug(
                "Certificate '%s' is not marked as a CA in its basic "
...
def verify_certificate(context, certificate_uuid,
    ...
    Load the set of trusted certificates and the certificate to validate from
    the key manager and store the trusted certificates in a certificate
...
class CertificateVerificationContext(object):
    ...
    def __init__(self, certificate_tuples):
        ...
        for certificate_tuple in certificate_tuples:
            ...
            if can_sign_certificates(certificate, certificate_uuid):
                self._signing_certificates.append(certificate_tuple)
            else:
                LOG.warning(
                    "Certificate '%s' is not configured to act as a signing "
                    "certificate. It will not be used as a signing "
                    "certificate.",
                    certificate_uuid)
    ...
    def verify(self):
        ...
        Locate the signed certificate's signing certificate in the context
        certificate cache, using both subject/issuer name matching and
        signature verification. Check that if the certificate is self-signed
        it is also located in the context certificate cache. Construct the
        certificate chain from certificates in the context certificate cache.
        Verify that the each signing certificate can have a sufficient number
        of child certificates to support the chain.
        ...
        while True:
            ...
            if signing_certificate_tuple:
                ...
                if signed_certificate == signing_certificate_tuple[1]:
                    ...
                    certificate_chain.insert(
                        0, signing_certificate_tuple[1])
                    certificate_chain_uuids.insert(
                        0, signing_certificate_tuple[0])
                    signed_certificate = signing_certificate_tuple[1]
            else:
                uuid = certificate_chain_uuids[0]
                raise exception.SignatureVerificationError(
                    "Certificate chain building failed. Could not locate the "
                    "signing certificate for %s in the set of context "
                    "certificates." %
        ...
        for i in range(len(certificate_chain)):
            ...
            # The check for ExtensionNotFound should never be needed due to
            # context checks in init and during chain building. It is included
            ...
            try:
                ...
                raise exception.SignatureVerificationError(
                    ...
                    "'%s' is not configured to support certificate chains of "
                    "sufficient length." % certificate_chain_uuids[i]
                )
